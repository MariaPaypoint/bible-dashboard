...
4. **Assertions**: Проверяй наличие уникальных элементов UI (заголовки, лейблы), а не общий текст "Success".
5. **Headless**: Тесты должны стабильно работать в headless режиме (ждать элементы через `wait_for_selector`, а не фиксированные таймуты).
